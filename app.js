...
// Update the Product Search Dropdown
...
// Add Item to Temporary Transaction List
function addItemToTransaction() {
    ...
    if (!product || qty <= 0) return alert("Select a product and quantity.");
...
function renderPendingItems() {
    ...
    ).join('') + `<div style="margin-top:10px; border-top:1px solid #444; padding-top:5px;"><strong>Total: ${total.toLocaleString()} CFA</strong></div>`;
}
...
async function saveProduct() {
    ...
    if (editingProductId) {
        ...
        document.getElementById('p-title').innerText = "Inventory Update";
    } else {
...
// --- SAVE CUSTOMER (SALES & PAYMENTS) ---
async function saveCustomer() {
    const name = document.getElementById('c-name').value;
    const phone = document.getElementById('c-phone').value;
    const paid = parseFloat(document.getElementById('c-paid').value) || 0;
    
    let totalBill, itemsText, techItems;

    if (editingCustomerId) {
        const old = customers.find(c => c.id === editingCustomerId);
        // If updating payment without adding new items, keep old bill details
        totalBill = pendingItems.length > 0 ? pendingItems.reduce((s, i) => s + i.total, 0) : old.total_bill;
        itemsText = pendingItems.length > 0 ? pendingItems.map(i => `${i.qty} ${i.name}`).join(', ') : old.items_bought;
        techItems = pendingItems.length > 0 ? pendingItems.map(i => `${i.qty}:${i.productId}`).join('|') : old.items_technical;
    } else {
        if (pendingItems.length === 0) return alert("Add items to the list first!");
        totalBill = pendingItems.reduce((s, i) => s + i.total, 0);
        itemsText = pendingItems.map(i => `${i.qty} ${i.name}`).join(', ');
        techItems = pendingItems.map(i => `${i.qty}:${i.productId}`).join('|');
    }
...
function startEditCustomer(id) {
    ...
    document.getElementById('pending-items').innerHTML = `<div style="color:orange; font-size:0.8rem;">Bill: ${c.total_bill.toLocaleString()} CFA. Updating payment for: ${c.customer_name}</div>`;
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

// --- DELETE TRANSACTION (WITH AUTH) ---
async function deleteCustomerTransaction(id) {
    const auth = prompt("Are you sure? This will NOT restore stock automatically. Type 'yes' to delete:");
...
function renderUI() {
    ...
    document.getElementById('inventory-body').innerHTML = inventory.map(p => {
        ...
        return `<tr>
            ...
            <td style="color:${bal <= 1 ? 'var(--danger)' : 'white'}">${bal.toFixed(1)} Doz</td>
            <td><button onclick="startEditProduct(${p.id})" class="btn-edit" style="padding:4px 8px;">Edit</button></td>
        </tr>`;
    }).join('');
    ...
    document.getElementById('ledger-body').innerHTML = filtered.map(c => `<tr>
        ...
        <td><span style="color:${c.balance > 0 ? 'var(--danger)' : 'var(--success)'}; font-weight:bold;">${c.balance > 0 ? c.balance.toLocaleString() : 'CLEARED'}</span></td>
...
function clearCustomerForm() { 
    ...
    document.getElementById('c-title').innerText = "Sales & Payments";
}
