...
function updateProductDatalist() {
    // FIX: Included product name inside the value for visibility
    document.getElementById('product-list').innerHTML = inventory.map(p => {
        const bal = p.dozens - (p.sold_units || 0);
        return `<option value="${p.name}">${bal.toFixed(1)} Doz left | ${p.sell_price_cfa.toLocaleString()} CFA</option>`;
    }).join('');
}
...
function renderPendingItems() {
    ...
    box.innerHTML = pendingItems.map((item, index) => `<div>• ${item.qty} ${item.name} <span style="color:red; cursor:pointer;" onclick="removePendingItem(${index})">[x]</span></div>`).join('') + `<hr><strong>Total: ${total.toLocaleString()} CFA</strong>`;
}
...
async function saveProduct() {
    ...
    clearProductForm(); 
    await loadData(); // Force refresh to show changes
}

async function saveCustomer() {
    const name = document.getElementById('c-name').value;
    const paid = parseFloat(document.getElementById('c-paid').value) || 0;
    if (!name || (pendingItems.length === 0 && !editingCustomerId)) return alert("Missing Customer Name or Items!");
    ...
    clearCustomerForm(); 
    await loadData(); // Force refresh to update the display list immediately
}
...
function startEditCustomer(id) {
    ...
    editingCustomerId = id; document.getElementById('c-title').innerText = "Updating: " + c.customer_name;
    ...
    document.getElementById('pending-items').innerHTML = `<div style="color:orange">Currently owes: ${c.balance.toLocaleString()} CFA</div>`;
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

async function deleteCustomerTransaction(id) {
    // AUTHENTICATION: Requires typing 'yes'
    ...
    if (auth && auth.toLowerCase() === 'yes') {
        ...
        await loadData(); 
        alert("Stock restored and record deleted.");
    }
}

async function deleteProduct(id) {
    const auth = prompt("Type 'yes' to permanently delete this product from inventory:");
    if (auth && auth.toLowerCase() === 'yes') { 
...
function renderUI() {
    ...
    document.getElementById('inventory-body').innerHTML = inventory.map(p => {
        ...
        return `<tr>
            ...
            <td style="color:${stock <= 1 ? 'red' : 'white'}">${stock} Doz</td>
            <td>
                <button onclick="startEditProduct(${p.id})" class="btn-edit">Edit</button>
                <button onclick="deleteProduct(${p.id})" class="btn-del">X</button>
            </td>
        </tr>`;
    }).join('');
