...
window.startNewBatch = () => {
    const name = prompt("New Batch Name (e.g., Nov 2025):");
    if (name) {
        activeBatch = name;
...
window.saveProduct = async function() {
    const payload = {
        ...
        "total_naira": (parseFloat(document.getElementById('p-dozens').value) || 0) * (parseFloat(document.getElementById('p-naira').value) || 0),
        "total_expected_cfa": (parseFloat(document.getElementById('p-dozens').value) || 0) * (parseFloat(document.getElementById('p-sell').value) || 0)
    };
...
window.saveCustomer = async function() {
    ...
    if (!name || queue.length === 0) return alert("Select products first!");
    const { error } = await _db.from('customers').insert([{ name, phone, items_json: queue, total_amount: total, amount_paid: paid, balance: total - paid, updated_at: new Date().toISOString(), batch_tag: activeBatch }]);
    if (!error) { updateStock(queue); queue = []; loadData(); }
};
...
window.addToExistingCustomer = async function(id) {
    ...
    const { error } = await _db.from('customers').update({ items_json: newItems, total_amount: c.total_amount + addedTotal, balance: (c.total_amount + addedTotal) - c.amount_paid, updated_at: new Date().toISOString() }).eq('id', id);
    if (!error) { updateStock(queue); queue = []; loadData(); }
};
